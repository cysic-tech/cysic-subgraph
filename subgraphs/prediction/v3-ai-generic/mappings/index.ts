...
import {
  ...
  StartRound,
  Unpause,
} from "../generated/PredictionV3AI/PredictionV3AI";
...
export function handlePause(event: Pause): void {
  ...
  if (market === null) {
    ...
    market.totalTreasuryAmount = ZERO_BD;
    market.totalBetsClaimed = ZERO_BI;
    market.totalClaimedAmount = ZERO_BD;
    market.winRate = HUNDRED_BD;
    market.averageAmount = ZERO_BD;
    market.netAmount = ZERO_BD;
    market.save();
  }
  market.epoch = event.params.epoch.toString();
  market.paused = true;
...
export function handleBetBull(event: BetBull): void {
  ...
  market.totalAmount = market.totalAmount.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  market.totalBullAmount = market.totalBullAmount.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  market.winRate = market.totalBetsClaimed.divDecimal(market.totalBets.toBigDecimal()).times(HUNDRED_BD);
  market.averageAmount = market.totalAmount.div(market.totalBets.toBigDecimal());
  market.netAmount = market.netAmount.minus(event.params.amount.divDecimal(EIGHTEEN_BD));
  market.save();
  ...
  if (user === null) {
    ...
    user.totalAmount = ZERO_BD;
    user.totalBullAmount = ZERO_BD;
    user.totalBearAmount = ZERO_BD;
    user.totalBetsClaimed = ZERO_BI;
    user.totalClaimedAmount = ZERO_BD;
    user.winRate = HUNDRED_BD;
    user.averageAmount = ZERO_BD;
    user.netAmount = ZERO_BD;
  ...
  user.totalAmount = user.totalAmount.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  ...
  if (user.totalBets.gt(ZERO_BI)) {
    ...
    user.averageAmount = user.totalAmount.div(user.totalBets.toBigDecimal());
  }
  user.netAmount = user.netAmount.minus(event.params.amount.divDecimal(EIGHTEEN_BD));
  user.save();
...
export function handleBetBear(event: BetBear): void {
  ...
  market.totalAmount = market.totalAmount.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  market.totalBearAmount = market.totalBearAmount.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  market.winRate = market.totalBetsClaimed.divDecimal(market.totalBets.toBigDecimal()).times(HUNDRED_BD);
  market.averageAmount = market.totalAmount.div(market.totalBets.toBigDecimal());
  market.netAmount = market.netAmount.minus(event.params.amount.divDecimal(EIGHTEEN_BD));
  market.save();
...
export function handleClaim(event: Claim): void {
  ...
  if (bet === null) {
    ...
    if (bet.amount.gt(ZERO_BD)) {
      bet.claimedNetAmount = event.params.amount.divDecimal(EIGHTEEN_BD).minus(bet.amount);
    }
    bet.updatedAt = event.block.timestamp;
    bet.save();
  }
  ...
  user.totalClaimedAmount = user.totalClaimedAmount.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  if (user.totalBets.gt(ZERO_BI)) {
    user.winRate = user.totalBetsClaimed.divDecimal(user.totalBets.toBigDecimal()).times(HUNDRED_BD);
  }
  user.netAmount = user.netAmount.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  user.save();
  ...
  market.totalClaimedAmount = market.totalClaimedAmount.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  market.winRate = market.totalBetsClaimed.divDecimal(market.totalBets.toBigDecimal()).times(HUNDRED_BD);
  market.netAmount = market.netAmount.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  market.save();
}
...
export function handleRewardsCalculated(event: RewardsCalculated): void {
  ...
  market.totalTreasuryAmount = market.totalTreasuryAmount.plus(event.params.treasuryAmount.divDecimal(EIGHTEEN_BD));
  market.save();
}
